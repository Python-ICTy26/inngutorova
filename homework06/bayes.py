        ...
        for i in range(len(texts)):
            ...
            for type in self.types:
                ...
                for word in texts[i].split():
                    prob += math.log((self.words_enters[type][word] + self.alpha) / ( sum(self.words_enters[type].values()) + self.alpha * self.words_count)) #вероятность слова в типе как количество встреч слова в типе / количество всех встреч слов в типе
                if prob > max_prob:
                    max_prob = prob
                    max_prob_type = type
