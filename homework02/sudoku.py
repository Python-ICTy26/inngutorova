...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    """
    block = []
    ll = int(math.sqrt(len(grid)))
    for i in range(ll):
        for j in range(ll):
            block.append(grid[(pos[0] // ll) * ll + i][(pos[1] // ll) * ll + j])
    return block
...
def find_possible_values(
    ...
    for i in range(1, len(grid) + 1):
        if (
            str(i) not in get_col(grid, pos)
            and str(i) not in get_row(grid, pos)
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    """
    gr: tp.Optional[tp.List[tp.List[str]]] = grid

    pos = find_empty_positions(gr)
    if not pos:
        return gr
    else:
        for i in find_possible_values(gr, pos):
            gr[pos[0]][pos[1]] = str(i)
            if solve(gr):
                return gr
            else:
                gr[pos[0]][pos[1]] = "."
